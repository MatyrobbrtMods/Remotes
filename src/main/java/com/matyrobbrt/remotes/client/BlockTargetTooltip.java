...
public record BlockTargetTooltip(BlockTarget target, Component message, ItemStack renderInstance) implements ClientTooltipComponent {
    ...
    public void renderText(Font pFont, int pX, int pY, Matrix4f pMatrix4f, MultiBufferSource.BufferSource pBufferSource) {
        ...
        pFont.drawInBatch(this.message, pX, pY + textOffset, -1, true, pMatrix4f, pBufferSource, Font.DisplayMode.NORMAL, 0, 15728880);
    }
    ...
    public void renderImage(Font pFont, int pMouseX, int pMouseY, PoseStack pPoseStack, ItemRenderer pItemRenderer) {
        if (!renderInstance.isEmpty()) {
            pItemRenderer.renderAndDecorateItem(pPoseStack, renderInstance, pMouseX + pFont.width(message) + 4, pMouseY);
        }
    }
}
